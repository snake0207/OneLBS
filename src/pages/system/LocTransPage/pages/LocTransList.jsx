...
const LocTransList = () => {
    ...
    const handleSearch = (values) => {
        const sdate = `${values.start_date.split('-').join('')}${values.start_time}`
        const edate = `${values.end_date.split('-').join('')}${values.end_time}`
        console.log(sdate, edate)
        setFetchData({ count: 0, lists: [] })
        setIsSearchClick((prev) => !prev)
        setQueryParams({ ...queryParams, start_date: sdate, end_date: edate, page: 1 })
    }
